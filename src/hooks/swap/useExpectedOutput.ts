...
const useExpectedOutput = () => {
  ...
  const [minimumAmountOutResult, setMinimumAmountOutResult] = useState<
    string | undefined
  >();
  const [amountInResult, setAmountInResult] = useState<
    ethers.BigNumber | string
  >();

  const [maxOut, setMaxOut] = useState<ethers.BigNumber | string>();
  const [formattedAmountOutResult, setFormattedAmountOutResult] = useState<
    string | undefined
  >();
  const { SwapperV2Proxy_CONTRACT } = useCallContract();
  ...
  useEffect(() => {
    async function getExOutput() {
      ...
      if (
        ...
        if (
          token0.address.toLowerCase() === WTON_ADDRESS.toLowerCase() ||
        ...
        if (
          ...
          account &&
          params &&
          Number(fromAmountIn) !== 0 &&
          QUOTER_CONTRACT &&
          fromAmountIn !== undefined
        ) {
          const amountOut = await QUOTER_CONTRACT.callStatic.quoteExactInput(
            params.path,
            amountIn
          );
          ...
          if (
            ...
            const formatted =
              converted && converted.indexOf(".") > -1
                ? converted?.slice(0, converted?.indexOf(".") + 3)
                : converted;
            ...
            const formattedAmountOut =
              convertedAmountOut && convertedAmountOut.indexOf(".") > -1
                ? convertedAmountOut?.slice(
                    0,
    ...
    async function getExpectedOutMax() {
      let denominator;
      let numerator;
      const int = Number.isInteger(Number(slippage));
    //   console.log('token0Balance',token0Balance);
      ...
      if (
        ...
        const amountIn = ethers.utils.parseEther(token0Balance);
        const params = getParams(token0.address, token1.address);
        if (
          library &&
          account &&
