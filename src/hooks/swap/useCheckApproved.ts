...
const useCheckApproved = () => {
    ...
    const { TON_ADDRESS, WTON_ADDRESS, TOS_ADDRESS, SwapperV2Proxy } = CONTRACT_ADDRESS;
        ...
        async function getApproved() {
            ...
            if (WTON_CONTRACT && Token0Contract) {
                if (token0.address.toLowerCase() === WTON_ADDRESS) {
                    ...
                    const allowAmount = await WTON_CONTRACT.allowance(account, SwapperV2Proxy)
                    ...
                    const allowAmount = await Token0Contract.allowance(account, SwapperV2Proxy)
                    const converted = convertNumber({
                        amount: allowAmount.toString(),
                        localeString: false,
                    });
